...
//            ps.setObject(1, hoaDonID);
    ...
    public ArrayList<HoaDonResponse1> getAllByStatus() {
        String sql = "SELECT  hd.id ,hd.id_khach_hang ,hd.id_nhan_vien ,\n"
                + "                     hd.trang_thai ,hd.tong_tien ,hd.ma_hoa_don,\n"
                + "                     kh.ho_ten ,nv.ma_nhan_vien ,nv.ten_nhan_vien ,\n"
                + "                     hd.ngay_tao \n"
                + "                   FROM HoaDon hd ,KhachHang kh ,NhanVien nv \n"
                + "                   WHERE  hd.id_khach_hang  = kh.id \n"
                + "                   AND hd.id_nhan_vien  = nv.id \n"
                + "                     AND hd.trang_thai = 0";
    ...
    public boolean updateThongTin(HoaDonResponse1 response) {
        ...
        // Câu lệnh SQL không bao gồm việc cập nhật hình thức thanh toán
        String sql = "UPDATE HoaDon SET trang_thai = 1 WHERE id = ?";

        try (Connection con = DBConnect.getConnection();
                PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setObject(1, response.getId());  // Đặt id
    ...
    public boolean huyHoaDon(HoaDonResponse1 hoaDon) {
        int check = 0;
        String sql = "UPDATE HoaDon SET trang_thai = 2 WHERE id = ?";
        try (Connection con = DBConnect.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setObject(1, hoaDon.getId()); // Sử dụng ID của hóa đơn để xác định bản ghi cần cập nhật
            check = ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace(System.out);
        }
